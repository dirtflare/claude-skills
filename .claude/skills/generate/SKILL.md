...
| `--limit N` | 今回落とす件数の上限。まず少数で動作と容量を確かめる |
| `--allow-host H` | 接続先をこのホストに限定 (複数指定可)。既定は制限なし |
| `--dry-run` | 件数と保存先だけ表示 |

実行すると、送信先ホストの一覧を必ず表示する。マニフェストは外部から来た
JSON なので、**どこへ繋ぎに行くかは口頭の説明ではなく、この出力で確認する。**
意図しないホストが混ざっていたら `--allow-host` で絞る。
